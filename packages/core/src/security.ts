...
const secretKey =
  /^(?:password|passwd|secret|credential|authorization|api[-_]?key|token|access[-_]?token|refresh[-_]?token|auth[-_]?token|thinking[-_]?signature|signature|encrypted[-_]?content)$/i;
